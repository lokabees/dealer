  ...
  <div class="bg-warning-light bg-opacity-25 sm:flex py-10">
    ...
    <div class="flex-initial max-w-lg text-center text-warning m-auto prose">
      <span class="font-bold">{{
        $t('dashboard.complete_shop_prompt.title')
      }}</span
      ><br />
      <span>{{ $t('dashboard.complete_shop_prompt.text') }}</span
      ><br />
      <button
        class="bg-warning text-white py-2 mt-2"
        @click="$router.push('/shop/edit')"
      >
        {{ $t('dashboard.complete_shop_prompt.button') }}
      </button>
    </div>
